...
IrrKlangSceneNode::IrrKlangSceneNode(irrklang::ISoundEngine* soundEngine, 
...
IrrKlangSceneNode::~IrrKlangSceneNode()
{
	stop();
...
void IrrKlangSceneNode::OnRegisterSceneNode()
{
...
void IrrKlangSceneNode::OnAnimate(u32 timeMs)
{
...
const core::aabbox3d<f32>& IrrKlangSceneNode::getBoundingBox() const
{
	return Box;
}
...
void IrrKlangSceneNode::serializeAttributes(io::IAttributes* out, io::SAttributeReadWriteOptions* options) const
{
	if (!out)
		return;
...
void IrrKlangSceneNode::deserializeAttributes(io::IAttributes* in, io::SAttributeReadWriteOptions* options)
{
	if (!in)
		return;
...
ISceneNode* IrrKlangSceneNode::clone(ISceneNode* newParent, ISceneManager* newManager)
{
	...
	IrrKlangSceneNode* nb = new IrrKlangSceneNode(SoundEngine, newParent, newManager, ID);
...
ESCENE_NODE_TYPE IrrKlangSceneNode::getType() const
{
	return (ESCENE_NODE_TYPE)IRRKLANG_SCENE_NODE_ID;
}


void IrrKlangSceneNode::stop()
{
...
void IrrKlangSceneNode::setPlayOnceMode(bool deleteWhenFinished)
{
	stop();
	PlayMode = EPM_ONCE;
...
void IrrKlangSceneNode::setLoopingStreamMode()
{
	stop();
...
void IrrKlangSceneNode::setRandomMode(int minTimeMs, int maxTimeMs)
{
	stop();
	PlayMode = EPM_RANDOM;
...
void IrrKlangSceneNode::setSoundFileName(const char* soundFilename)
{
	if (soundFilename)
...
const char* IrrKlangSceneNode::getSoundFileName() const
{
...
void IrrKlangSceneNode::setMinMaxSoundDistance(f32 minDistance, f32 maxDistance)
{
...
IrrKlangSceneNodeFactory::IrrKlangSceneNodeFactory(irrklang::ISoundEngine* sengine, ISceneManager* mgr)
...
IrrKlangSceneNodeFactory::~IrrKlangSceneNodeFactory()
{
	if (SoundEngine)
		SoundEngine->drop();
}
...
ISceneNode* IrrKlangSceneNodeFactory::addSceneNode(ESCENE_NODE_TYPE type, ISceneNode* parent)
{
	if (!parent)
	...
	{
		IrrKlangSceneNode* node = new IrrKlangSceneNode(SoundEngine, parent, Manager, -1);
		node->drop();
		return node; 
	}
...
ISceneNode* IrrKlangSceneNodeFactory::addSceneNode(const c8* typeName, ISceneNode* parent)
{
...
u32 IrrKlangSceneNodeFactory::getCreatableSceneNodeTypeCount() const
{
	return 1;
}
...
ESCENE_NODE_TYPE IrrKlangSceneNodeFactory::getCreateableSceneNodeType(u32 idx) const
{
	if (idx==0)
		return (ESCENE_NODE_TYPE)IRRKLANG_SCENE_NODE_ID;
...
const c8* IrrKlangSceneNodeFactory::getCreateableSceneNodeTypeName(u32 idx) const
{
	if (idx==0)
...
//! returns type name of a createable scene node type 
const c8* IrrKlangSceneNodeFactory::getCreateableSceneNodeTypeName(ESCENE_NODE_TYPE type) const
{
	if (type == IRRKLANG_SCENE_NODE_ID)
		return irrKlangSceneNodeTypeName;
...
ESCENE_NODE_TYPE IrrKlangSceneNodeFactory::getTypeFromName(const c8* name)
{
