...
Animation::Animation()
{
	this->animations = new map<string,AnimationEntry *>();
}

void Animation::addAnimation(AnimationEntry *ae)
{	
	animations->insert(std::pair<string,AnimationEntry *>(*(ae->getName()),ae));
}

AnimationEntry *Animation::getAnimationEntry(char *p)
{
	ae = animations->find(std::string(p))->second;	
	return ae;
}